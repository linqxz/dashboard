...
def safe_to_datetime(timestamps, default_unit="ms"):
    ...
    if hasattr(timestamps, "dtype") and pd.api.types.is_datetime64_any_dtype(
        timestamps
    ):
        return timestamps
    ...
    if hasattr(timestamps, "__iter__") and not isinstance(timestamps, str):
        first_valid = (
            timestamps.dropna().iloc[0]
            if hasattr(timestamps, "dropna")
            else next((t for t in timestamps if pd.notna(t)), None)
        )
        if first_valid is not None and isinstance(
            first_valid, (pd.Timestamp, datetime)
        ):
            return timestamps
    ...
    if hasattr(timestamps, "__iter__") and not isinstance(timestamps, str):
        non_null_ts = (
            timestamps.dropna()
            if hasattr(timestamps, "dropna")
            else [t for t in timestamps if pd.notna(t)]
        )
...
def get_healthy_databases():
    ...
    for db_path, status in st.session_state.databases_status.items():
        ...
        if status.get("healthy") == True:  # noqa: E712
        ...
        # Handle different status formats as fallback
            ...
            if status["status"].get("general_status") == True:  # noqa: E712
                healthy_dbs.append(db_path)
    return healthy_dbs
...
def load_trades_data(db_path: str, limit: int = 100, offset: int = 0):
    ...
    try:
        trades = backend_client.archived_bots.get_database_trades(
            db_path, limit, offset
        )
...
def load_orders_data(
    db_path: str, limit: int = 100, offset: int = 0, status: str = None
):
    """Load orders data with pagination"""
    try:
        orders = backend_client.archived_bots.get_database_orders(
            db_path, limit, offset, status
        )
...
def load_positions_data(db_path: str, limit: int = 100, offset: int = 0):
    """Load positions data"""
    try:
        positions = backend_client.archived_bots.get_database_positions(
            db_path, limit, offset
        )
...
def get_trade_analysis(db_path: str):
    ...
    try:
        ...
        exchanges = (
            df["connector_name"].unique().tolist()
            if "connector_name" in df.columns
            else []
        )
        trading_pairs = (
            df["trading_pair"].unique().tolist() if "trading_pair" in df.columns else []
        )
...
def create_bot_runs_scatterplot(bot_runs: List[Dict], healthy_databases: List[str]):
    ...
    for run in bot_runs:
        try:
            ...
            for controller_name, controller_perf in performance.items():
                if (
                    isinstance(controller_perf, dict)
                    and "performance" in controller_perf
                ):
            ...
            duration_hours = (
                (stopped_at - deployed_at).total_seconds() / 3600
                if deployed_at and stopped_at
                else 0
            )

            # Check if database is available
            has_database = any(
                run.get("bot_name", "") in db for db in healthy_databases
            )
    ...
    for has_db, label, color, symbol in [
        (True, "With Database", "#4CAF50", "circle"),
        ...
        if not subset.empty:
            fig.add_trace(
                go.Scatter(
                    ...
                    name=label,
                    marker=dict(
                        size=subset["duration_hours"].apply(
                            lambda x: max(8, min(x * 2, 50))
                        ),
                        color=color,
                        symbol=symbol,
...
def get_historical_candles(
    ...
    try:
        ...
        buffer_time = timedelta(
            hours=1
        )  # 2 hours buffer for 20 candles at 5min = 100 minutes
...
def create_trades_chart(trades_data: List[Dict[str, Any]]):
    ...
    fig.add_trace(
        go.Bar(x=daily_volume["date"], y=daily_volume["amount"], name="Daily Volume")
    )
...
def create_comprehensive_dashboard(
    ...
    fig = make_subplots(
        ...
        subplot_titles=(f"{trading_pair} Price & Trades", "PnL & Fees vs Position"),
        ...
        for idx, trade in trades_df.iterrows():
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name=f"{'Buy' if is_buy else 'Sell'} Trade",
                    showlegend=False,
                    hovertemplate=f"<b>{'Buy' if is_buy else 'Sell'}</b><br>Price: ${trade['price']:.4f}<br>Amount: "
                                  f"{trade.get('amount', 0):.4f}<extra></extra>",
                ),
                row=1,
                col=1,
            )
...
if st.session_state.bot_runs:
    scatterplot_result = create_bot_runs_scatterplot(
        st.session_state.bot_runs, healthy_databases
    )
...
with col3:
    load_dashboard_btn = st.button(
        "Load Dashboard",
        use_container_width=True,
        type="primary",
        disabled=not st.session_state.selected_database,
    )
...
if st.session_state.selected_database:
    ...
    if st.session_state.db_summary and matching_bot_run:
        ...
        deployed_date = (
            pd.to_datetime(matching_bot_run.get("deployed_at", "")).strftime(
                "%m/%d %H:%M"
            )
            if matching_bot_run.get("deployed_at")
            else "N/A"
        )

        st.info(
            f"{bot_name} | {strategy} | Deployed: {deployed_date} | {summary.get('total_trades', 0)} trades on "
            f"{summary.get('exchanges', ['N/A'])[0]} {summary.get('trading_pairs', ['N/A'])[0]}"
        )
    ...
    if (
        st.session_state.db_performance
        and st.session_state.trades_data
        and st.session_state.trade_analysis
    ):
        ...
        with col2:
            ...
            st.metric("Total Fees (Quote)", value=f"${fees:,.4f}")
        ...
        with col4:
            ...
            st.metric("Total Volume", value=f"${volume:,.2f}")
        ...
        if analysis.get("exchanges") and analysis.get("trading_pairs"):
            ...
            with col1:
                selected_exchange = (
                    analysis["exchanges"][0]
                    if len(analysis["exchanges"]) == 1
                    else st.selectbox("Exchange", options=analysis["exchanges"])
                )

                selected_pair = (
                    analysis["trading_pairs"][0]
                    if len(analysis["trading_pairs"]) == 1
                    else st.selectbox("Trading Pair", options=analysis["trading_pairs"])
                )
            ...
            if (
                st.session_state.get("candle_key") != candle_key
                and analysis.get("start_time")
                and analysis.get("end_time")
            ):
            ...
            if st.session_state.historical_candles:
                ...
                trades_list = (
                    trades_data.to_dict("records") if not trades_data.empty else []
                )
            ...
            st.warning("No trading data found for analysis.")
    else:
