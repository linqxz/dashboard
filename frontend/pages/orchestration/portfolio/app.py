...
from frontend.st_utils import download_csv_button, get_backend_api_client, initialize_st_page
from frontend.visualization.theme import get_default_layout

initialize_st_page(title="Portfolio", icon=None, ms_icon="finance")
...
def aggregate_portfolio_history(history_df, time_window_seconds=10):
    ...
    history_df["time_group"] = history_df["timestamp"].dt.floor(
    ...
    for time_group in history_df["time_group"].unique():
        ...
        agg_group = (
            group_data.groupby(["account", "exchange", "token"])
            .agg(
                {
                    "value": "sum",
                    "units": "sum",
...
def get_portfolio_filters():
    ...
    selected_accounts = st.multiselect(
        "Select Accounts", accounts_list, accounts_list, key="main_accounts"
    )
        ...
        if account in portfolio_state:
            for exchange in selected_exchanges:
                if exchange in portfolio_state[account]:
                    tokens_available.extend(
                        [info["token"] for info in portfolio_state[account][exchange]]
                    )

    tokens_available = list(set(tokens_available))
    selected_tokens = st.multiselect(
        "Select Tokens", tokens_available, tokens_available, key="main_tokens"
    )

    return selected_accounts, selected_exchanges, selected_tokens


# Get filters once at the top level (inside an expander to save vertical space)
...
def portfolio_overview():
            ...
            for exchange in selected_exchanges:
                if exchange in portfolio_state[account]:
                    filtered_portfolio_state[account][exchange] = [
                        token_info
                        for token_info in portfolio_state[account][exchange]
    ...
    with c1:
        ...
        portfolio_df["label"] = (
            portfolio_df["token"]
            + " ($"
            + portfolio_df["value"].apply(lambda x: f"{x:,.2f}")
            + ")"
        )
        ...
        fig.update_layout(
            **{
                **get_default_layout(title="Portfolio Allocation", height=600),
    ...
    with c2:
        ...
        fig.update_layout(**get_default_layout(title="Token Distribution", height=600))
        fig.update_layout(xaxis_title="Token", yaxis_title="Value (USD)")
        st.plotly_chart(fig, use_container_width=True)

    # Keep the dataframe rendering out of the overview; moved to Details tab
    st.session_state["_portfolio_df"] = portfolio_df[
        ["account", "exchange", "token", "units", "price", "value", "available_units"]
    ]
...
def portfolio_history():
    ...
    with col1:
        days_back = st.selectbox(
            "Time Period", [7, 30, 90, 180, 365], index=1, key="history_days"
        )
    with col2:
        limit = st.number_input(
            "Max Records", min_value=10, max_value=1000, value=100, key="history_limit"
        )
    with col3:
    ...
    history_df = aggregate_portfolio_history(
        history_df, time_window_seconds=time_window
    )
    ...
    fig.update_layout(
        **get_default_layout(title="Portfolio Value Evolution by Account", height=400)
    )
    fig.update_layout(xaxis_title="Time", yaxis_title="Value (USD)")
    st.plotly_chart(fig, use_container_width=True)

    # Portfolio evolution by token (area chart)
    st.subheader("Portfolio Evolution by Token")
    token_evolution_df = (
        history_df.groupby(["timestamp", "token"])["value"].sum().reset_index()
    )
    token_evolution_df = token_evolution_df.sort_values("timestamp")

    # Show only top 10 tokens by average value to avoid clutter
    top_tokens = (
        token_evolution_df.groupby("token")["value"]
        .mean()
        .sort_values(ascending=False)
        .head(10)
        .index
    )
    token_evolution_filtered = token_evolution_df[
        token_evolution_df["token"].isin(top_tokens)
    ]

    fig = px.area(
        token_evolution_filtered,
        x="timestamp",
        y="value",
        color="token",
    ...
    fig.update_layout(
        **get_default_layout(
            title="Portfolio Value Evolution by Token (Top 10)", height=400
        )
    )
    fig.update_layout(xaxis_title="Time", yaxis_title="Value (USD)")
    st.plotly_chart(fig, use_container_width=True)
    ...
    exchange_evolution_df = (
        history_df.groupby(["timestamp", "exchange"])["value"].sum().reset_index()
    )
    ...
    fig.update_layout(
        **get_default_layout(title="Portfolio Value Evolution by Exchange", height=400)
    )
    fig.update_layout(xaxis_title="Time", yaxis_title="Value (USD)")
    st.plotly_chart(fig, use_container_width=True)
    ...
    evolution_table["timestamp"] = evolution_table["timestamp"].dt.strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    evolution_table["value"] = evolution_table["value"].round(2)
    evolution_table = evolution_table.rename(
        columns={"timestamp": "Time", "value": "Total Value (USD)"}
    )
    st.dataframe(evolution_table, use_container_width=True)


# Details tab (table + CSV)
@st.fragment
def portfolio_details():
    st.markdown("---")
    st.subheader("Portfolio Details")
    # Use cached dataframe from overview if available; otherwise recompute quickly
    df = st.session_state.get("_portfolio_df")
    if df is None:
        try:
            portfolio_state = client.portfolio.get_state(
    ...
    download_csv_button(
        df, filename="portfolio_details", key="download_portfolio_details"
    )


# Main portfolio page
overview_tab, history_tab, details_tab = st.tabs(["Overview", "History", "Details"])
with overview_tab:
