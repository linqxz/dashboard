...
initialize_st_page(
    title="Grid Strike Grid Component",
    icon=None,
    ms_icon="grid_view",
...
fig = make_subplots(
    ...
    cols=1,
    subplot_titles=(
        f"Grid Strike Grid Component - {inputs['trading_pair']} ({inputs['interval']})",
    ),
)
...
grid_traces = get_grid_trace(
    inputs["start_price"], inputs["end_price"], inputs["limit_price"]
)
...
def prepare_config_for_save(config):
    ...
    if (
        "triple_barrier_config" in prepared_config
        and prepared_config["triple_barrier_config"]
    ):
        for key in [
            "open_order_type",
            ...
            if (
                key in prepared_config["triple_barrier_config"]
                and prepared_config["triple_barrier_config"][key] is not None
            ):
                prepared_config["triple_barrier_config"][key] = prepared_config[
