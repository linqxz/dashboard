...
def user_inputs():
    ...
    trading_pairs = default_config.get(
    ...
    trailing_stop_activation_pct = default_config.get(
        "trailing_stop_activation_pct", None
    )
    trailing_stop_trailing_delta_pct = default_config.get(
    ...
    if not all_trading_pairs:
        with st.expander("Specific Trading Pairs", expanded=True):
            ...
            if trading_pairs_input.strip():
                trading_pairs = [
                    pair.strip()
                    for pair in trading_pairs_input.split(",")
                    if pair.strip()
                ]
            else:
                trading_pairs = []
            ...
            st.info(
                f"Monitoring {len(trading_pairs)} trading pairs: {', '.join(trading_pairs[:5])}"
    ...
    with st.expander("Risk Management (Triple Barriers)", expanded=True):
        ...
        with c3:
            ...
            use_trailing_stop = st.checkbox(
                ...
                value=(
                    trailing_stop_activation_pct is not None
                    and trailing_stop_trailing_delta_pct is not None
                    and trailing_stop_activation_pct > 0
                    and trailing_stop_trailing_delta_pct > 0
                ),
            ...
            if use_trailing_stop:
                trailing_stop_activation_pct = st.number_input(
                    ...
                    min_value=0.1,
                    max_value=100.0,
                    value=trailing_stop_activation_pct
                ...
                trailing_stop_trailing_delta_pct = st.number_input(
                    ...
                    min_value=0.1,
                    max_value=100.0,
                    value=trailing_stop_trailing_delta_pct
                    if trailing_stop_trailing_delta_pct is not None
                    else 2.0,
                    step=0.1,
    ...
    with st.expander("Configuration Summary", expanded=False):
        ...
        st.write(
            f"- Time Limit: `{time_limit}s` ({'Immediate close' if time_limit == 0 else 'Barriers enabled'})"
        )
        st.write(
            f"- Stop Loss: `{stop_loss_pct}%`"
            if stop_loss_pct
            else "- Stop Loss: `Disabled`"
        )
        st.write(
        ...
        if trailing_stop_activation_pct and trailing_stop_trailing_delta_pct:
            st.write(
                f"- Trailing Stop: `{trailing_stop_activation_pct}%` / `{trailing_stop_trailing_delta_pct}%`"
            )
        else:
