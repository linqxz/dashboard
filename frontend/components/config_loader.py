...
def get_default_config_loader(controller_name: str):
    ...
    loader_key = f"config_loader_initialized_{controller_name}"  # noqa F841
            ...
            if not use_default_config:
                ...
                if len(configs) > 0:
                    ...
                    if selected_config:
                        ...
                        st.session_state[config_key]["controller_name"] = (
                            controller_name
                        )
                else:
...
def reset_controller_config(controller_name: str) -> None:
    ...
    if (
        st.session_state.get("default_config", {}).get("controller_name")
        == controller_name
    ):
